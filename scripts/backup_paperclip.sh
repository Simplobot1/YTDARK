...
#
# O tar é dividido em chunks de 45MB (limite de upload do Supabase Storage).
# Restore: baixar os chunks, `cat paperclip-<stamp>.tar.gz.part* > backup.tar.gz`
#          e extrair sobre /opt/paperclip-data com o container parado.
#
...
echo "[backup] $(date '+%F %T') — empacotando config do Paperclip"
tar czf "$WORK/paperclip-${STAMP}.tar.gz" -C "$DATA_DIR" \
...
SIZE_MB="$(du -m "$WORK/paperclip-${STAMP}.tar.gz" | cut -f1)"
echo "[backup] tamanho: ${SIZE_MB}MB — dividindo em chunks de 45MB"
split -b 45M -d "$WORK/paperclip-${STAMP}.tar.gz" "$WORK/paperclip-${STAMP}.tar.gz.part"

for part in "$WORK"/paperclip-"${STAMP}".tar.gz.part*; do
  name="$(basename "$part")"
...
curl -sf -X POST "${SUPABASE_URL}/storage/v1/object/list/${BUCKET}" \
  ...
  grep -oE '"name":"paperclip-[0-9]{8}-[0-9]{4}\.tar\.gz\.part[0-9]+"' |
  cut -d'"' -f4 |
  while read -r old; do
...
echo "[backup] ok: paperclip-${STAMP}.tar.gz (${SIZE_MB}MB em chunks)"
